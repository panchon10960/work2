...
with codecs.open("item_category_20180120220513.csv", "r", "Shift-JIS", "ignore") as file:
...
with transaction.atomic():
    for i, product in enumerate(product_id_list):
        ...
        # ikeda
        # buyma_product = Buyma.objects.filter(title=product_list[i]).values().first()

        # buyma_product = Buyma.objects.filter(title=product_list[i].replace('「大特価」', '').replace('☆SALE☆ ', '').replace('「破格」 ', '')).values().first()
        # saki
        ...
        if buyma_product is None and math.isnan(category_id1_list[i]) == True:
            ...
            if False == math.isnan(category_id1_list[i]):
                ...
                product_dict['category_id1'] = category_map[buyma_product['category_name']]
